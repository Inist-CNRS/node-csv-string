...
exports.read = function (input, sep, callback) {
  ...
  while (startingIndex <= input.length) {
    ...
    token = trim(input.slice(startingIndex, endingIndex));
      ...
      startingIndex = endingIndex + 1;
       ...
       if (s.indexOf('"') < 0) {
         ...
         if (i === 0 /* -1 + 1 */) {
          ...
          var j = input.slice(startingIndex + i + 2).indexOf(sep) + 1;
          var k = input.slice(startingIndex + i + 2 + j).search('[^\\s]+.');
          startingIndex += i + 2 + ((j + k) >= 0 ? j + k : 0);
        }
    }
